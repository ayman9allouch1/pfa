...
class NewModelForm extends React.Component {
  ...
  handleFileChange = (e) => {
    ...
    const validExtensions = [".h5", ".pkl", ".pickle", ".model", ".joblib", ".pt"];
  ...
  createModel = e => {
    e.preventDefault();
    axios.post('http://127.0.0.1:8000/api/segmentation-models/', this.state, 
      {headers: { 
  ...
  editModel = e => {
    e.preventDefault();
    axios.patch('http://127.0.0.1:8000/api/segmentation-models/' + this.state.id, this.state, {
      headers: { 
        'content-type': 'multipart/form-data',
        Authorization: `Bearer ${this.props.access}` 
      }})
